...
namespace DreamExcel.Core
{
    public class WorkBookCore
    {
        ...
        {
            var destFile = Config.ExePath + "/" + Path.GetFileNameWithoutExtension(path);
            Console.WriteLine(path);
            Console.WriteLine(destFile);
            File.Copy(path, destFile, true);
            using (var stream = File.Open(destFile, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                try
                {
                        ...
                        for (var i = 0; i < table.Count; i++)
                        {
                            ...
                            else
                            {
                                ...
                                if (t.Type == "enum")
                                {
                                    ...
                                    if (x != null)
                                        core.Data = sheet.GetRow(TypeRow).GetCell(t.Colunm).CellComment.String.String
                                            .Replace("\r", "").Split(new[] {"\n"},
                    ...
                    try
                    {
                        if (Config.Info.GeneratorType == "DB")
                        {
                            ...
                            {
                                WriteDB(node, fileName, keyType, table, columnCount, rowCount, passColumns, sheet);
                            }
                        }
                        else if (Config.Info.GeneratorType == "Json")
                        {
                            foreach (var node in Config.GetDBPath(fileName))
                            {
                                WriteJson(node, fileName, newType, keyIndex, columnCount, rowCount, passColumns,
                                    sheet);
                            }
                        }
                    }
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if (property.PropertyType == typeof(BigInteger))
                                property.SetValue(instance, BigInteger.Parse(sheet.GetRow(j).GetCell(i).ToString()));
                            else
                            {
